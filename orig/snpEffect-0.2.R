...
SNP_EFFECT_CODING <- SNP_EFFECT[c("CODING", "SYNONYMOUS_CODING", 
...
function(snp, 
         gff, 
         genome, 
         species="no.spp.given", 
         #outgrp=read.DNAStringSet("E_consensusCP.fa"), 
         outgrp=read.DNAStringSet("E_consensus_noIRA.fa"), 
         outgrp.name="P.tremula",
         nHaplotypes=20, 
         report.interval=10, 
         ...)
{
  ...
  for (i in 1:nrow(snp)) {
    ...
    if (length(locus_tags) > 0) {
      for (tag in locus_tags) {
        ...
        thissnpEffect <- singleSNPEffect(thissnp=snp[i, ], 
            feat=thisfeat, 
            genome=genome, 
            species=species, 
            outgrp=outgrp, 
            outgrp.name=outgrp.name, 
...
# thissnp is position etc. of snp, features is overlap of gff
{
  ...
  if (ref != refref && SNP.ref.must.match.genome) {
    cat("SNP @", pos, ": SNP reference allele and reference genome do not match\n")
  }
  outgrp.allele <- as.character(subseq(outgrp, start=pos, width=1))
  change <- as.character(thissnp$SNPAllele)
  changefreq <- thissnp$NumStrains / nHaplotypes
  effect.feat <- feat
  if ("CDS" %in% feat$type) {
  ...
  posWindow <- sequenceWindowAtPos(genome, pos, change, lengthWindowSide=lengthWindowSide)
  ...
  ans <- data.frame(
    ...
    position=pos,
    ref=I(ref),
    outgrp=I(outgrp.allele),
    outgrp.name=I(outgrp.name),
    change=I(change),
    changetype=I("SNP"),
    Homozygous=I("Hap"),
...
# produce string of characters surrounding genome position
sequenceWindowAtPos <- 
function(genome, pos, change="", lengthWindowSide=10, sep="|")
{
  w1start <- max(pos - lengthWindowSide, 1)
  w1width <- pos - w1start
  w2start <- pos + 1
  w2width <- min(width(genome) - pos, lengthWindowSide)
  ans <- paste(sep=sep,
  ...
  for (i in 1:nrow(snp)) {
    ...
    if (length(locus_tags) > 0) {
      for (tag in locus_tags) {
        ...
        thisIndelEffect <- singleIndelEffect(thisindel=snp[i, ], 
            feat=thisfeat, 
            genome=genome, 
            nHaplotypes=nHaplotypes, 
            ...)
...
# thisindel is position etc. of snp, features is overlap of gff
{
  ...
  change.indel <- thisindel$Indel # thisindel$VarAllele
  change.seq <- thisindel$Sequence # thisindel$VarAllele
  change = paste(sep="", change.indel, change.seq)
  refref <- as.character(subseq(genome, start=pos, width=1))
  changefreq <- thisindel$Proportion # thisindel$VarFreq
  effect.feat <- feat
  if ("CDS" %in% feat$type) {
    # at least one hit is coding
    effect <- SNP_EFFECT["CODING"]
    ...
    while (TRUE) {
      ...
      if (translation == "") {
        ...
        if (CDS$strand == "+") {
          ...
          ref.codon.rc <- subseq(genome, start=seq.codon.pos, width=3)
          snp.codon.rc <- ref.codon.rc
          subseq(snp.codon.rc, start=(3 - CDS.phase), width=1) <- change.seq
          ref.codon <- as.character(reverseComplement(ref.codon.rc))
          snp.codon <- "###" # as.character(reverseComplement(snp.codon.rc))
          if (debug) 
            cat("  - seq.codon.pos", seq.codon.pos, "ref.codon", ref.codon, "snp.codon", snp.codon, "\n")
